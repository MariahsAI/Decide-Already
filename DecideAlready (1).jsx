...
const DecideAlready = () => {
  ...
  const [phase, setPhase] = useState('loading'); // loading, onboarding, ask, commit, reveal, feedback
  ...
  const [userProfile, setUserProfile] = useState({
    ...
    chronotype: '', // earlyBird, nightOwl, flexible
    fitnessLevel: '', // beginner, intermediate, advanced
    datingStyle: '', // casual, serious, notLooking
  });
  ...
  // Extended decisions with tags for filtering
  const decisions = {
    eat: [
  ...
  useEffect(() => {
    ...
    // Check if reroll was used today
  ...
  const getDecision = () => {
    ...
    // Filter out previously rejected decisions
    ...
    // Fallback if all options filtered out
    if (options.length === 0) {
